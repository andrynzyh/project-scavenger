...
export function resolveHomepageIcon(name: string | undefined, folder: 'digimon' | 'accessories'): string | undefined {
  ...
  // If it looks like a path (contains / or extension), normalize onto the base
  if (name.includes('/') || IMAGE_EXT_RE.test(name)) {
    return withBase(name.startsWith('/') ? name : `/${name}`);
  }
  ...
  if (match) {
    // Return path relative to root: /<base>/digimon/filename.png
    return withBase(`/${folder}/${match}`);
  }
  ...
  if (slugMatch) return withBase(`/${folder}/${slugMatch}`);
